...
export class DepthScenery {
  ...
  inactiveModels(stage: number): T.Object3D[] {
    const active = new Set(SCENERY_SCHEDULE[stage]?.map((pass) => pass.model));
    return [...this.models].filter(([name]) => !active.has(name)).map(([, group]) => group);
  }
